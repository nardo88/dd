...
export class PhpBase extends EventTarget {
  constructor(PhpBinary, args = {}) {
    ...
    this.binary = new PhpBinary(Object.assign({}, defaults, phpSettings, args, fixed))
      .then((php) => {
        const retVal = php.ccall('pib_init', NUM, [STR], [])
  ...
  run(phpCode) {
    return this.binary
      .then((php) => php.ccall('pib_run', NUM, [STR], [`?>${phpCode}`], { async: true }))
      .finally(() => this.flush())
  }

  exec(phpCode) {
    return this.binary
      .then((php) => php.ccall('pib_exec', STR, [STR], [phpCode], { async: true }))
      .finally(() => this.flush())
  }

  tokenize(phpCode) {
    return this.binary.then((php) =>
      php.ccall('pib_tokenize', STR, [STR], [phpCode], { async: true })
    )
  }
